...
const STATUT_COLORS = {
  PLANIFIEE:     "bg-blue-600 text-white",
  TENUE:         "bg-emerald-500 text-white",
  ANNULEE:       "bg-red-500 text-white",
  PAS_DE_RETOUR: "bg-slate-400 text-white",
};
const STATUT_BORDER = {
  PLANIFIEE:     "border-l-blue-500",
  TENUE:         "border-l-emerald-500",
  ANNULEE:       "border-l-red-500",
  PAS_DE_RETOUR: "border-l-slate-400",
};
const STATUT_HEADER = {
  PLANIFIEE:     "bg-blue-50",
  TENUE:         "bg-emerald-50",
  ANNULEE:       "bg-red-50",
  PAS_DE_RETOUR: "bg-slate-50",
};
...
function BureauModal({ ag, onClose }) {
  ...
  const [editing,   setEditing]   = useState(false);  // true = modif bureau existant

  // form nouveau/modif mandat
  ...
  const openEditBureau = () => {
    if (!bureau) return;
    setDateDebut(bureau.date_debut || "");
    setDateFin(bureau.date_fin || "");
    const existing = [...(bureau.membres || [])].map(m => ({
      personne_id: String(m.personne),
      fonction: m.fonction,
    }));
    setMembres(existing);
    setEditing(true);
  ...
  const handleSave = async () => {
    ...
    try {
      let mandatId;
      if (editing && bureau) {
        // PATCH mandat existant
        const res = await fetch(`/api/mandats-bureau/${bureau.id}/`, {
        ...
        mandatId = bureau.id;
        // Supprimer tous les membres existants puis recréer
        for (const m of bureau.membres || []) {
          await fetch(`/api/membres-bureau/${m.id}/`, {
        ...
        mandatId = (await res.json()).id;
      }
      for (const m of membres) {
        await fetch("/api/membres-bureau/", {
          method: "POST", credentials: "include",
          headers: { "Content-Type": "application/json", "X-CSRFToken": getCsrf() },
          body: JSON.stringify({ mandat: mandatId, personne: m.personne_id, fonction: m.fonction }),
        });
      }
      setShowForm(false); setEditing(false);
      fetchBureau();
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl mt-8 mb-8">
        ...
        <div className="p-6 min-h-[320px]">
          {loading ? (
            ...
            /* Formulaire création / modification (prioritaire sur la vue bureau) */
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between pt-2">
                <button onClick={() => { setShowForm(false); setEditing(false); setError(""); setMembres([]); }}
                ...
                <button onClick={handleSave} disabled={saving}
                  ...
                  {saving ? "Enregistrement…" : (editing ? "Enregistrer les modifications" : "Créer le bureau")}
                </button>
              </div>
            </div>
          ) : bureau ? (
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                ...
                <button onClick={openEditBureau}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-slate-100 text-slate-700 text-xs font-semibold hover:bg-slate-200 transition">
        ...
        <div ref={menuRef} className="flex flex-col gap-3">
          {items.map(item => {
            const dateFormatted = item.date_ag
              ...
              : item.date_ag;
            return (
            <div key={item.id} className={`bg-white rounded-xl border border-slate-200 border-l-4 shadow-sm flex flex-col gap-0 relative overflow-hidden ${STATUT_BORDER[item.statut] ?? "border-l-slate-300"}`}>
              {/* Header coloré */}
              <div className={`flex items-start justify-between gap-2 px-4 py-3 ${STATUT_HEADER[item.statut] ?? "bg-slate-50"}`}>
                <div className="flex items-center gap-2 flex-wrap min-w-0">
                  <span className="font-bold text-slate-800 text-sm">
                    AG du {dateFormatted}
                  </span>
                  <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full ${STATUT_COLORS[item.statut] ?? "bg-slate-400 text-white"}`}>
                    {item.statut_label}
                  </span>
                </div>
