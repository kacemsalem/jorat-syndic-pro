    ...
    <div className="max-w-7xl mx-auto space-y-4">
      ...
      {lots.length > 0 && (
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: "Total lots",    value: stats.total,                  color: "bg-slate-100 text-slate-700 border border-slate-200" },
            { label: "À jour",        value: stats.ajour,                  color: "bg-green-50  text-green-700  border border-green-200" },
            { label: "En retard",     value: stats.retard,                 color: "bg-red-50    text-red-700    border border-red-200" },
            { label: "Reste à payer", value: `${fmt(stats.restant)} MAD`,  color: "bg-amber-50  text-amber-700  border border-amber-200" },
          ].map((s, i) => (
            <div key={i} className={`flex items-center justify-between rounded-xl px-4 py-2 ${s.color} shadow-sm`}>
