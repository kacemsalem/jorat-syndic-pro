...
export default function ModelesDepensePage() {
  ...
  const filtered = useMemo(() => {
    let list = modeles;
    if (filterFamille) list = list.filter(m => String(m.famille_depense) === filterFamille);
    ...
    <div className="bg-slate-100 min-h-screen -m-3 sm:-m-6 pb-24">
      ...
      <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 px-4 pt-5 pb-14">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-white/70 text-[10px] font-bold uppercase tracking-widest">Dépenses</p>
            <h1 className="text-white font-bold text-xl leading-tight">Modèles de dépenses</h1>
          </div>
        ...
        <p className="text-white/70 text-xs">Templates pré-remplis pour la saisie rapide</p>
        <p className="text-2xl font-bold text-white mt-1">
          {modeles.filter(m => m.actif).length} <span className="text-base font-normal text-white/60">modèle(s) actif(s)</span>
        </p>
      </div>
